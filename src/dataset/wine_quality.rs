...
fn parse_data_file(data_file: &str) -> Result<(Vec<String>, Vec<String>, Vec<WineQuality>), DatasetParseError> {
    ...
    let mut target_headers = Vec::<String>::new();
    for (n, l) in data_lines.enumerate() {
        if let Ok(s) = l {
            if n == 0 {
                feature_headers = s.split(";").map(|v| v.to_string()).collect();
                target_headers.push(feature_headers.pop().unwrap());
            } else {
    ...
    Ok((feature_headers, target_headers, wine_recs))
}

fn load_dataset(data_file: &str) -> Result<Dataset<f32, f32>, DatasetParseError> {
    let (feature_headers, target_headers, wine_records) = parse_data_file(data_file)?;
    let mut x: Vec<f32> = Vec::new();
    let mut y: Vec<f32> = Vec::new();
    let num_features = feature_headers.len();
...
mod tests {
    ...
    use crate::dataset::wine_quality::{
        WINE_RED_DATASET_SAMPLES_SIZE,
        WINE_RED_DATASET_NO_FEATURES
    };
