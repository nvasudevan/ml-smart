...
impl<'a> DBScanRun<'a> {
    ...
    //     let nm_train = DenseMatrix::from_array(
    //         self.tt_ds.y_train.len(),
    //         self.tt_ds.ds.num_features,
    //         &self.tt_ds.x_train.to_row_vector());
    //     let nm_test = DenseMatrix::from_array(
    ...
    //             let model = DBSCAN::fit(&self.tt_ds.x_train, params)?;
    //             model.predict(&self.tt_ds.x_test)?
    //         }
    ...
    //                 .with_algorithm(algo_params.algorithm.clone())
    //                 .with_eps(algo_params.eps);
    //             let model = DBSCAN::fit(&nm_train, params)?;
    //             model.predict(&nm_test)?
    //         }
    //         _ => {
    //             let params = DBSCANParameters::default()
    //                 .with_algorithm(algo_params.algorithm.clone())
